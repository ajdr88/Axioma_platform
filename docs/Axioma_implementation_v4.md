...
**Companion document:** Axioma_requirements_v4.md, Axioma_stage_tracking_amendment.md
**Change basis:** Folds in all twelve prioritized architecture-review findings (Rev B, carried forward) plus the Rev C addition: Program & Subsystem Lifecycle Tracking (FR-PM-01…05). Material changes flagged **[REV-B §x]** or **[REV-C]**.
...
> **Pending additions (not yet merged — see `docs/claude/`):** `:Constraint`/`:Parameter`/`:InformationElement`/`:Interaction`/`:InteractionFragment`/`:Collection` + `Derive`/`Copy`/`Bound`/`member` edges (`Axioma_gap_closure_amendment.md`); `:Function`/`:SelectionChoice`/`:ConnectionChoice` + `derives`/`incompatibleWith`/`choiceConstraint` edges (`Axioma_turbofan_system_model_amendment.md` Part 3); `citation`/`confidence` properties + `:CandidateStructureSuggestion` (`Axioma_document_import_pipeline_amendment.md`).
...
> **Pending ADR candidates (not yet accepted — see `docs/claude/`):** ADR-009 (Interaction/timing SysML v2 mapping, `Axioma_gap_closure_amendment.md`), ADR-010 (report-template mechanism, same doc), ADR-011 — **note: two different unmerged amendments each independently proposed "ADR-011"**: `Axioma_document_import_pipeline_amendment.md` proposes it for confirming `llm-gateway` as a Product-1-tier dependency, while `Axioma_turbofan_system_model_amendment.md` §3.6 proposes it (later updated by `Axioma_sysml_tool_landscape_evaluation.md` §2.2) for adopting `adsg-core`+`SBArchOpt` as Mode B's design-space/optimizer foundation. **These need renumbering before either is accepted** — see `docs/IMPLEMENTATION_KICKOFF.md` Phase 0.
...
> See `docs/claude/Axioma_turbofan_system_model_amendment.md` Part 1 §1.3 for worked Fan & LP Compression / Core (HP) Compressor examples extending this table.
...
> **Note:** `docs/claude/Axioma_turbofan_system_model_amendment.md` Part 3 §3.7 flags that P2.1's Mo 7–9 estimate was set before the FR-ARCH gap (design-space/optimizer representation) was identified, and should be re-scoped once ADR-011 (§2.5 above) is resolved.
