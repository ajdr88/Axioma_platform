...
**[REV-D, implemented `docs/IMPLEMENTATION_KICKOFF.md` Phase 5 — see impl v5 §13]:** FR-CORE-16's review-gate UI now surfaces `origin` at all — `AutonomyPanel.tsx`'s `Proposal` type previously didn't declare the field even though the API has returned it since Phase 1, so the client silently discarded it. Now rendered as a badge plus an origin filter.

**[REV-D, implemented `docs/IMPLEMENTATION_KICKOFF.md` Phase 5 — see impl v5 §15 for full detail]:** FR-CORE-14…18 are now built as a real async pipeline (`POST /import/documents` + 3 `GET`s + `POST .../proposal`, `apps/api/src/document_import.rs`) — real per-page PDF text extraction (`pdf-extract`, text layers only, **no OCR this pass**), deterministic "shall"-sentence segmentation with a length-based confidence heuristic, real Ollama-drafted `{name, shallText, category}` per candidate (citation/provenance stamped), and a real `document-import`-origin proposal (one per completed job, batching every drafted candidate — reqs v5 §5.6's own "consolidated-batch" option) that `mode_b.rs::accept_proposal` now genuinely materializes into real `:Requirement` elements. Confirmed end-to-end via both integration tests and a live `curl` multipart upload through accept. FR-CORE-17's suggestions are a simple heuristic (Title-Case word runs), not real NLP — flagged, not oversold.
...
**[REV-D, implemented `docs/IMPLEMENTATION_KICKOFF.md` Phase 5 — see impl v5 §14 for full detail]:** research before building this found none of the three mechanisms this group's own design text (§5.12) assumes already exist actually did — FR-SAFE-05's export was a single hardcoded pipeline, no "Generic Table view" exists, and `GeometryPointer` was dead code with `ObjectStore` write-only. FR-EXPORT-01 is built client-side only (current viewport, `html-to-image`) — the server-side headless full-diagram render is not attempted. FR-EXPORT-02 is CSV only (no XLSX crate yet), scoped by `NodeKind` or a frozen Collection rather than a nonexistent Generic Table view. FR-EXPORT-03 is a real, minimal template mechanism with exactly one registered template (`"risk-register"`, generalizing FR-SAFE-05's own export for the first time) rendered as HTML, not PDF. FR-EXPORT-04 is fully built — `ObjectStore` gained its first-ever read method.
